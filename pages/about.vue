<template>
  <section class="About_Hero">
    <div class="container">
      <div class="row">
        <div class="col-lg-6">
          <div class="d-flex flex-column gap-3 pe-lg-3">
            <div class="d-flex flex-column pe-lg-5" data-aos="fade-up">
              <h2>DiscountsLK</h2>
              <h4>About</h4>
              <p class="pe-lg-5">
                Welcome to DiscountsLK, Sri Lanka’s top platform for local
                savings! Our mission is to connect you with unbeatable discounts
                from nearby stores, restaurants, and services—all within a 5KM
                radius. With just a few taps, you can discover exclusive deals
                and start saving.
              </p>
              <p class="mt-3 pe-lg-5">
                <b>Why Choose DiscountsLK?</b>
                Finding deals has never been easier. Search by category or
                location to access fantastic offers tailored to your needs.
                Whether you're dining, shopping, or exploring new experiences,
                DiscountsLK makes it easy to enjoy the best of Sri Lanka while
                saving on what you love most.
              </p>
            </div>
            <hr data-aos="fade-up" />
            <div
              class="d-flex gap-5 justify-content-lg-start justify-content-center mb-lg-0 mb-3"
              data-aos="fade-up"
            >
              <div class="d-flex flex-column text-center gap-2">
                <h2><span class="number" data-final-value="135"></span>M+</h2>
                <p>App Downloads</p>
              </div>
              <div class="d-flex flex-column text-center gap-2">
                <h2><span class="number" data-final-value="200"></span>M+</h2>
                <p>User Reviews</p>
              </div>
            </div>
            <div
              class="d-flex gap-sm-4 gap-2 mt-lg-4 justify-content-lg-start justify-content-center"
            >
              <div>
                <a href="#" class="hover1">Download</a>
              </div>
              <div>
                <a href="Contact.html" class="hover2">Contact Us</a>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-6">
          <figure
            class="w-100 d-flex justify-content-center"
            data-aos="zoom-in-up"
            data-aos-delay="300"
          >
            <img
              src="/images/About/Benefits_Mobile.webp"
              alt="About_Mobile"
              data-aos="zoom-in-up"
            />
          </figure>
          <div
            class="d-flex align-items-center justify-content-center position-relative"
          >
            <div
              class="Price_1st d-flex flex-column gap-3"
              data-aos="zoom-in-up"
              data-aos-delay="200"
            >
              <div>
                <i class="fa-solid fa-circle-check fa-xl"></i>
              </div>
              <div>
                <p>save upto</p>
                <h2>
                  $<span class="number" data-final-value="2864">2864</span>
                </h2>
                <p>to Find your Smart Home</p>
              </div>
            </div>
            <div
              class="credit position-absolute"
              data-aos="zoom-in-up"
              data-aos-delay="200"
            >
              <i class="fa-regular fa-credit-card"></i>
            </div>
            <div
              class="home position-absolute"
              data-aos="zoom-in-up"
              data-aos-delay="200"
            >
              <i class="fa-solid fa-house"></i>
            </div>
            <div class="Dotted_Line position-absolute">
              <img src="/images/icon/dotedLine_Stats.svg" alt="dotedLine" />
            </div>
          </div>
        </div>
      </div>
    </div>
    <div class="avater_Images mt-lg-4 mt-3">
      <div class="member">
        <div class="position-relative">
          <img src="/images/About/Hero_member-1.webp" alt="Hero_member" />
          <img src="/images/About/Hero_member-2.webp" alt="Hero_member" />
          <img src="/images/About/Hero_member-3.webp" alt="Hero_member" />
          <img src="/images/About/Hero_member-4.webp" alt="Hero_member" />
        </div>
        <p>
          Join Over <span class="number" data-final-value="4400">0</span>+
          Members
        </p>
      </div>
    </div>
  </section>
  <!-- ======== End of 2.1. About_Hero section ========  -->
  <!-- ======== 1.4. Partners section ========  -->
  <section class="Partners">
    <h2 class="d-none">Lorem, ipsum dolor</h2>
    <div class="container">
      <hr />
      <div class="Partners_Slider" data-aos="zoom-in">
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo1.webp" alt="logo1"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo2.webp" alt="logo2"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo3.webp" alt="logo3"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo4.webp" alt="logo4"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo5.webp" alt="logo5"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo6.webp" alt="logo6"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo1.webp" alt="logo7"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo2.webp" alt="logo8"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo3.webp" alt="logo9"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo4.webp" alt="logo10"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo5.webp" alt="logo11"
          /></a>
        </div>
        <div>
          <a href="#"
            ><img src="/images/About/Partners_logo6.webp" alt="logo12"
          /></a>
        </div>
      </div>
      <hr />
    </div>
  </section>
  <!-- ======== End of 1.4. Partners section ========  -->
  <!-- ======== 1.5. Services section ========  -->
  <section class="Services About_Services">
    <div class="container">
      <h2 data-aos="fade-up">Exclusive Offers with Sponsored Discounts</h2>
      <p class="Services_p" data-aos="fade-up">
        Our Sponsored Discounts feature highlights top deals from local
        businesses, so you never miss out on the latest offers. We curate the
        best discounts for easy access, helping you stay ahead of every exciting
        promotion right at your fingertips.
      </p>
      <div class="row d-flex gap-md-0 gap-4 mt-lg-5">
        <div class="col-lg-4 col-md-4 d-flex">
          <div class="Services_Card" data-aos="flip-left">
            <div class="mb-2">
              <img src="/images/icon/Easily_Manage.png" alt="Manage" />
            </div>
            <h4>Boost Business Visibility</h4>
            <p>
              For businesses, reach more customers and drive foot traffic with
              exclusive offers.
            </p>
            <div class="mt-2">
              <a class="hover4" href="Features.html">Learn More</a>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-md-4 d-flex">
          <div class="Services_Card" data-aos="flip-left">
            <div class="mb-2">
              <img src="/images/icon/Find_Easily.svg" alt="Find_Easily" />
            </div>
            <h4>Quick Local Savings</h4>
            <p>
              Easily find exclusive deals within a 5KM radius, saving you time
              and money.
            </p>
            <div class="mt-2">
              <a class="hover4" href="Features.html">Learn More</a>
            </div>
          </div>
        </div>
        <div class="col-lg-4 col-md-4 d-flex">
          <div class="Services_Card" data-aos="flip-left">
            <div class="mb-2">
              <img src="/images/icon/Quick_Messege.svg" alt="Quick_Messege" />
            </div>
            <h4>Top Deals Highlighted</h4>
            <p>
              Sponsored Discounts showcase the best local offers, handpicked for
              you.
            </p>
            <div class="mt-2">
              <a class="hover4" href="Features.html">Learn More</a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
  <!-- ======== End of 1.5. Services section ========  -->
  <!-- ======== 1.8. Rating section ========  -->
  <section class="Rating pb-0">
    <div class="container">
      <div class="d-flex justify-content-around align-items-center">
        <div
          class="d-flex flex-column align-items-center justify-content-center gap-lg-3 gap-md-3 gap-2"
          data-aos="fade-up"
        >
          <h1><span class="number" data-final-value="162">0</span>+</h1>
          <p>App Downloads</p>
        </div>
        <hr />
        <div
          class="d-flex flex-column align-items-center justify-content-center gap-lg-3 gap-md-3 gap-2"
          data-aos="fade-up"
        >
          <h1><span class="number" data-final-value="182">0</span>+</h1>
          <p>Satisfied Clients</p>
        </div>
        <hr />
        <div
          class="d-flex flex-column align-items-center justify-content-center gap-lg-3 gap-md-3 gap-2"
          data-aos="fade-up"
        >
          <h1><span class="number" data-final-value="188">0</span>+</h1>
          <p>Brands Joined</p>
        </div>
        <hr />
        <div
          class="d-flex flex-column align-items-center justify-content-center gap-lg-3 gap-md-3 gap-2"
          data-aos="fade-up"
        >
          <h1><span class="number" data-final-value="166">0</span>+</h1>
          <p>Successful Campaigns</p>
        </div>
      </div>
    </div>
  </section>
  <!-- ======== End of 1.8. Rating section ========  -->
  <!-- ======== 1.10. GetApp section ========  -->
  <section class="GetApp mb-lg-5 mb-0">
    <div class="container">
      <div class="row">
        <div class="col-lg-6 col-md-6 position-relative">
          <div class="GetApp_Mobiles">
            <img
              src="/images/About/GetApp_Mobile1.webp"
              alt="GetApp_Mobile1"
              data-aos="zoom-in-up"
              data-aos-delay="200"
            />
            <img
              src="/images/About/GetApp_Mobile2.webp"
              alt="GetApp_Mobile2"
              data-aos="zoom-in-up"
              data-aos-delay="200"
            />
            <img
              src="/images/bg/GetApp_BG.webp"
              alt="GetApp_Circle"
              data-aos="zoom-in-up"
              data-aos-delay="500"
            />
          </div>
        </div>
        <div class="col-lg-6 col-md-6">
          <div data-aos="fade-up">
            <h1>Download Latest Version Of The App From</h1>
            <p class="GetApp_P">
              Ready to start saving? Download the latest version of the
              DiscountsLK app today and enjoy instant access to exclusive local
              discounts wherever you go. Simply download the app and unlock
              incredible savings at your favorite stores.
            </p>
            <div
              class="d-flex gap-lg-4 gap-3 justify-content-md-start justify-content-center"
            >
              <a href="#">
                <div>
                  <div
                    class="d-flex Social align-items-center gap-2 hover2 w-100"
                  >
                    <div><i class="fa-brands fa-google-play"></i></div>
                    <div>
                      <p>GET IN ON</p>
                      <h5>Google Play</h5>
                    </div>
                  </div>
                </div>
              </a>
              <a href="#">
                <div>
                  <div class="d-flex align-items-center gap-2 hover2 w-100">
                    <div><i class="fa-brands fa-apple"></i></div>
                    <div>
                      <p>Download on the</p>
                      <h5>App Store</h5>
                    </div>
                  </div>
                </div>
              </a>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
  <!-- ======== End of 1.10. GetApp section ========  -->
  <!-- ======== 2.2. Team section ========  -->
  <section class="Team">
    <div class="container">
      <div class="row">
        <div class="text-center mb-md-5" data-aos="fade-up">
          <h2>Our Team</h2>
          <p class="mt-3">
            Lorem ipsum dolor sit amet, consectetuer adipiscing elit.
          </p>
        </div>
        <div class="col-lg-3 col-md-3 col-sm-6">
          <div
            class="d-flex justify-content-center align-items-center flex-column position-relative"
            data-aos="flip-left"
          >
            <figure class="w-100">
              <img
                class="w-100"
                src="/images/About/Team_Member_1.webp"
                alt="Team_Member"
              />
            </figure>
            <div
              class="p-lg-4 p-3 position-absolute bottom-0 w-100 text-center d-flex flex-column"
            >
              <h4>Peter</h4>
              <p>Senior Developer</p>
              <div class="Team_social-icons d-flex justify-content-center">
                <a href="#" target="_blank"
                  ><i class="fab fa-facebook-f"></i
                ></a>
                <a href="#" target="_blank"><i class="fab fa-instagram"></i></a>
                <a href="#" target="_blank"><i class="fab fa-twitter"></i></a>
                <a href="#" target="_blank"
                  ><i class="fab fa-linkedin-in"></i
                ></a>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-md-3 col-sm-6">
          <div
            class="d-flex justify-content-center align-items-center flex-column position-relative"
            data-aos="flip-left"
          >
            <figure class="w-100">
              <img
                class="w-100"
                src="/images/About/Team_Member_2.webp"
                alt="Team_Member"
              />
            </figure>
            <div
              class="p-lg-4 p-3 position-absolute bottom-0 w-100 text-center d-flex flex-column"
            >
              <h4>Walker</h4>
              <p>Senior Business Advisor</p>
              <div class="Team_social-icons d-flex justify-content-center">
                <a href="#" target="_blank"
                  ><i class="fab fa-facebook-f"></i
                ></a>
                <a href="#" target="_blank"><i class="fab fa-instagram"></i></a>
                <a href="#" target="_blank"><i class="fab fa-twitter"></i></a>
                <a href="#" target="_blank"
                  ><i class="fab fa-linkedin-in"></i
                ></a>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-md-3 col-sm-6">
          <div
            class="d-flex justify-content-center align-items-center flex-column position-relative"
            data-aos="flip-left"
          >
            <figure class="w-100">
              <img
                class="w-100"
                src="/images/About/Team_Member_3.webp"
                alt="Team_Member"
              />
            </figure>
            <div
              class="p-lg-4 p-3 position-absolute bottom-0 w-100 text-center d-flex flex-column"
            >
              <h4>Peter</h4>
              <p>Quality Assurnce</p>
              <div class="Team_social-icons d-flex justify-content-center">
                <a href="#" target="_blank"
                  ><i class="fab fa-facebook-f"></i
                ></a>
                <a href="#" target="_blank"><i class="fab fa-instagram"></i></a>
                <a href="#" target="_blank"><i class="fab fa-twitter"></i></a>
                <a href="#" target="_blank"
                  ><i class="fab fa-linkedin-in"></i
                ></a>
              </div>
            </div>
          </div>
        </div>
        <div class="col-lg-3 col-md-3 col-sm-6">
          <div
            class="d-flex justify-content-center align-items-center flex-column position-relative"
            data-aos="flip-left"
          >
            <figure class="w-100">
              <img
                class="w-100"
                src="/images/About/Team_Member_4.webp"
                alt="Team_Member"
              />
            </figure>
            <div
              class="p-lg-4 p-3 position-absolute bottom-0 w-100 text-center d-flex flex-column"
            >
              <h4>Devako</h4>
              <p>Designer</p>
              <div class="Team_social-icons d-flex justify-content-center">
                <a href="#" target="_blank"
                  ><i class="fab fa-facebook-f"></i
                ></a>
                <a href="#" target="_blank"><i class="fab fa-instagram"></i></a>
                <a href="#" target="_blank"><i class="fab fa-twitter"></i></a>
                <a href="#" target="_blank"
                  ><i class="fab fa-linkedin-in"></i
                ></a>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
  <!-- ======== End of 2.2. Team section ========  -->
  <!-- ======== 1.14. Community section ========  -->
  <section class="Community">
    <div
      class="d-flex justify-content-center align-items-center flex-column text-center gap-3 mb-lg-5 mb-4"
      data-aos="fade-up"
    >
      <h2>Join Our Global Community</h2>
      <p class="w-75 m-auto">
        Lorem Ipsum is simply dummy text of the printing and typesetting
        industry.
      </p>
    </div>
    <div class="mt-lg-4">
      <div
        class="d-flex justify-content-center align-items-center text-center gap-lg-5 gap-4 w-100 position-relative"
      >
        <div data-aos="flip-left">
          <h1><span class="number" data-final-value="200">0</span>M</h1>
          <h4>Customers</h4>
        </div>
        <div data-aos="flip-left">
          <h1><span class="number" data-final-value="935">0</span>M+</h1>
          <h4>Users</h4>
        </div>
        <div data-aos="flip-left">
          <h1><span class="number" data-final-value="24">0</span>K+</h1>
          <h4>Subscribers</h4>
        </div>
      </div>
    </div>
  </section>
  <!-- ======== End of 1.14. Community section ========  -->
  <!-- ======== 1.15. Register section ========  -->
  <section class="Register">
    <div class="container">
      <div class="row">
        <div class="col-lg-5 position-relative">
          <figure class="position-absolute" data-aos="zoom-in-up">
            <img
              src="/images/index/Register_Img.webp"
              alt="Register_Img"
              data-aos="zoom-in-up"
              data-aos-delay="200"
            />
          </figure>
        </div>
        <div class="col-lg-7">
          <div data-aos="fade-up">
            <h1 class="mb-lg-4 mb-3">Get The App Register Now!</h1>
            <p>
              Unlock exclusive local discounts with ease. Download the app,
              register, and start saving at your favorite nearby stores today!
            </p>
            <form
              action="https://html-templates.evonicmedia.com/"
              id="Subscribe-massage2"
              class="w-100 d-flex justify-content-between gap-2 align-items-center"
            >
              <input
                class="w-75 border-0"
                name="Email"
                type="email"
                placeholder="Enter Your Email For Registration"
                required
                autocomplete="on"
              />
              <button class="hover2" type="submit">Submit</button>
            </form>
            <div id="Succes-box2" class="validation"></div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>
